...
function actb(obj,evt,ca,noMSearch,nElemen){
	/* ---- Variables ---- */
	var actb_timeOut = -1; // Autocomplete Timeout in ms (-1: autocomplete never time out)
	var actb_lim = nElemen;    // Number of elements autocomplete can show (-1: no limit)
	var actb_firstText = noMSearch; // should the auto complete be limited to the beginning of keyword?
	...
	function verificaItem() {
		...
		if (actb_verificar) {
		for (i=0;i<actb_keywords.length;i++){
			if (actb_curr.value == actb_keywords[i]) {
				founded = true;
				break;
			}
		}
	...
	function actb_checkkey(evt){
		...
		switch (a){
			case 38:
				...
				break;
			case 40:
				//mod para abrir list ao pressionarmos down
				if (actb_display)
				{
					actb_godown();
					actb_caretmove = 1;
				...
				{
					if (actb_curr.value == '') actb_curr.value = ' ' ;
					setTimeout(function(){actb_tocomplete(32)},50);
				}
				break;
			case 13: case 9:
				actb_penter();
				actb_caretmove = 1;
				//mod para pular para o prox campo com tab.
				//return false;
				break;
				//mod para nao mostrar list com home,end,left,right
